...
parsing. JavaScript sends a fixed provider identifier, model identifier, visible conversation, the
current editor tree, optional focus, and a versioned sharing acknowledgement. It never receives a
key.
...
fail-soft convenience state. It contains no draft, focus, or hidden reasoning. An optional selected-
text focus is sent as a separate bounded plain-text field. It never becomes a file attachment or
durable editor range.
