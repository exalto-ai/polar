...
describe("reviewer connection manager", () => {
  ...
  it("keeps management available in Basic and atomically updates name and access", async () => {
    const state = statefulBridge([connection("connection-1")]);
    const controller = await openAndLoad(state.bridge, "basic");

    expect(document.querySelector<HTMLButtonElement>("#reviewer-add")!.hidden).toBe(true);
