...
export type ReviewerConnectionsController = {
  ...
  setMode(mode: "basic" | "connect" | null): void;
...
export function installReviewerConnections(
  ...
  const manager = required<HTMLElement>(root, "#reviewer-manager");
  ...
  let mode: "basic" | "connect" | null = null;
  ...
  function updateChrome(): void {
    manager.dataset.mode = mode ?? "unconfigured";
    addButton.hidden = mode !== "connect";
  ...
  function openAddFlow(): void {
    if (mode !== "connect" || bridge === null || documentContext === null) return;
    activeSetupId = null;
  ...
  return {
    ...
    setMode(nextMode) {
      mode = nextMode;
      if (mode !== "connect") closeAddFlow(false);
      updateChrome();
    },
