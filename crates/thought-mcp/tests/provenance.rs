...
fn a_later_model_on_another_document_cannot_rewrite_block_history() {
    ...
    assert_eq!(first_attribution.model.as_deref(), Some("model-a"));
    assert_eq!(second_attribution[0].model.as_deref(), Some("model-b"));
    ...
    let first_actor = ws
        ...
        .unwrap();
    assert_eq!(first_actor.model.as_deref(), Some("model-a"));
}
...
fn a_later_model_on_the_same_document_cannot_rewrite_an_untouched_block() {
    ...
    let by_id = ws
        ...
        .collect::<HashMap<_, _>>();
    assert_eq!(by_id[&earlier_block].model.as_deref(), Some("model-a"));
    assert_eq!(by_id[&changed_block].model.as_deref(), Some("model-b"));
