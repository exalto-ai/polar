...
impl ReviewerConnection {
    ...
    pub fn reported_mutation_context(&self, reported_model: Option<&str>) -> MutationContext {
        MutationContext::mcp_reported(
            format!("Configured for {}", self.client.configured_route_name()),
            Some(self.id.clone()),
            Some(self.provider.as_str().to_string()),
            reported_model.map(str::to_string),
        )
    }
}
...
mod tests {
    ...
    fn event_identity_never_reuses_a_model_from_an_earlier_call() {
        ...
        assert_eq!(context.reported_model(), None);
    }
