...
//! Doing that literally would give every client its own `polard`, and several
        ...
        match forward(&daemon, &line, &mut session) {
            ...
            Err(e) => {
                ...
                let error = serde_json::json!({
                    "jsonrpc": "2.0",
                    "id": id,
                    "error": { "code": -32603, "message": format!("polar daemon unreachable: {e}") }
                });
...
fn spawn() -> Result<Daemon, Box<dyn std::error::Error>> {
    ...
    let polard = exe.with_file_name("polard");
    ...
    Command::new(&polard)
        ...
        .stderr(Stdio::null())
        .spawn()
        .map_err(|e| format!("could not start {}: {e}", polard.display()))?;
