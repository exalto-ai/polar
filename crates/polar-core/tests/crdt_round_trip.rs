...
//!
//! This uses `polar_testkit::document`, the unconstrained generator, precisely
...
use polar_core::{Document, Position};
use polar_schema::{Mark, Node, Schema, normalize};
use polar_testkit::document;
...
fn insert_and_delete_address_blocks_by_id() {
    ...
    assert!(matches!(
        ...
        Err(polar_core::BlockError::NoSuchBlock(_))
    ));
}
