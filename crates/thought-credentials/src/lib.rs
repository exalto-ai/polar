//! Native storage for built-in provider API keys.
//!
//! Production macOS builds use the login Keychain. Tests use an explicit
//! owner-only file store. Keys never cross into the webview.
    ...
    fn trusted_application(path: &Path) -> Result<TrustedApplication, CredentialError> {
        let path = CString::new(path.as_os_str().as_bytes()).map_err(|_| {
            CredentialError::Platform(format!(
                "trusted application path contains a null byte: {}",
                path.display()
            ))
        })?;
        ...
        security_status("create trusted application", status)?;
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidStoredCredential => {
                formatter.write_str("stored provider credential is invalid")
            }
        }
    }
}
...
///
/// Only the app process needs these keys. The explicit file
...
fn credential_path(directory: &Path, provider_id: &str) -> PathBuf {
    directory.join(format!("{provider_id}.credential"))
}
...
fn write_private_file(
    ...
    let destination = credential_path(directory, provider_id);
    let result = (|| {
...
fn read_private_file(directory: &Path, provider_id: &str) -> Result<Vec<u8>, CredentialError> {
    let path = credential_path(directory, provider_id);
...
mod tests {
    ...
    fn file_store_refuses_credentials_with_broad_permissions() {
        ...
        let directory = tempfile::tempdir().unwrap();
        let store = ProviderCredentialStore::files(directory.path());
        store.set("openai", b"secret").unwrap();
        let path = directory.path().join("openai.credential");
