...
//!
//! `thoughtd` is a background process with no window. When something goes wrong
...
//! rest of its life.
//!
//! Logs go to a daily-rotated file under `THOUGHT_HOME` and, when someone is
//! watching, to stderr as well. `THOUGHT_LOG` overrides the level, e.g.
//! `THOUGHT_LOG=thought_mcp=debug,thoughtd=trace`.
...
pub fn init(home: &Path) -> LogGuard {
    ...
    let filter = || {
        EnvFilter::try_from_env("THOUGHT_LOG")
            .unwrap_or_else(|_| EnvFilter::new("info,thoughtd=info,thought_mcp=info"))
    };
