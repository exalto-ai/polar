...
/// Use a published daemon only when its private discovery capability works.
/// Actual MCP traffic always uses the reviewer-specific credential.
fn connect(
    connection_id: &str,
    credential: String,
    ...
    if let Some(daemon) = published {
        if !mcp_daemon_reachable(&daemon) {
            return Err(ShimFailure::new(
                ...
                format!(
                    "a thought daemon is already published but did not accept its private discovery capability; quit any running Proof of Thought or thoughtd process, then remove {} if the problem persists",
...
fn spawn() -> Result<Daemon, ShimFailure> {
    ...
    Err(ShimFailure::new(
        FailureCode::Transport,
        "daemon did not become reachable within 10 seconds",
    ))
}
...
/// Register the launcher before reading its credential. Identity and editor
/// health are verified first, so a forged discovery URL never receives the
/// editor bearer.
...
fn report_failure(reporter: Option<&FailureReporter>, code: FailureCode) {
    ...
    let _ = agent
        ...
        .header(
            "Authorization",
            &format!("Bearer {}", reporter.daemon.editor_token),
        )
        .header("Content-Type", "application/json")
        .send(&body);
}
...
mod tests {
    ...
    fn native_failure_payload_contains_only_non_secret_process_metadata() {
        ...
        let editor_capability = "editor-capability-sentinel";
        let reporter = FailureReporter {
            daemon: thoughtd::discovery::Daemon {
                ...
                pid: std::process::id(),
                token: "mcp-capability-sentinel".into(),
                editor_token: editor_capability.into(),
            },
        ...
        for secret in [old_reviewer_secret, new_reviewer_secret, editor_capability] {
