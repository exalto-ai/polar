...
impl Daemon {
    ...
    pub fn create_document_with_markdown(
        ...
        let body = match (title.trim(), initial_markdown) {
            ("", markdown) => {
                serde_json::json!({ "title": "Untitled", "initial_markdown": markdown.unwrap_or("") })
            }
            (title, Some(markdown)) => {
                serde_json::json!({ "title": title, "initial_markdown": markdown })
            }
        ...
        let mut response = self
            ...
            .header("Authorization", &format!("Bearer {}", self.editor_token))
            .send_json(body)
