...
fn prepare_failure_reporter(daemon: &Daemon, instance_id: &str) -> i64 {
    ...
    let mut response = post_json(&url, &daemon.editor_token, &body).unwrap();
...
#[test]
fn only_the_native_editor_capability_can_report_schema_failures() {
    ...
    for bearer in [&daemon.reviewer_token, &daemon.token] {
        assert!(matches!(
    ...
    assert!(matches!(
        post_json(
            &url,
            &daemon.editor_token,
            &failure_body("arbitrary", instance_id),
        ),
...
fn a_cold_launch_starts_the_daemon_before_reporting_a_missing_credential() {
    ...
    let base = discovery["url"]
        ...
        .strip_suffix("/mcp")
        .unwrap();
    let editor_token = discovery["editor_token"].as_str().unwrap();
